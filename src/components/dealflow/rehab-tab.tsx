    ...
    return (
        <div className="space-y-2 text-xs">
            {tasks.map(task => {
                ...
                const width = Math.max(0, ((end.getTime() - start.getTime()) / (totalDays * 1000 * 60 * 60 * 24)) * 100);
...
export function RehabTab({ deal, dealFlowData, updateDealFlow }: RehabTabProps) {
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <form onSubmit={handleSubmit(onSave)} className="lg:col-span-2 space-y-6">
                <Card>
                    <CardHeader>
                        <CardTitle>Rehab Tasks & Timeline</CardTitle>
                    </CardHeader>
                    ...
                    <CardFooter className="flex justify-between items-center">
                        <div className="text-lg font-bold">Total Cost: ${totalCost.toLocaleString()}</div>
